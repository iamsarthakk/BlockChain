...
bool block_t::validate()
{
    ...
    SHA256_CTX sha256y;
    SHA256_Init(&sha256y);
    for(auto txt: transactions) {
        if (!txt -> validate()){
            return false;
        }
        else{
        if (!(balances[txt->source_addr] >= txt->amount )){
            return false;
        }
        txt->update_balances(balances);
        SHA256_Update(&sha256y, txt->tx_hash.data(), txt->tx_hash.size());
      }
    }

    hash_result_t hash;
    SHA256_Update(&sha256y, reward_addr.data(), reward_addr.size());  // hash reward address
    SHA256_Update(&sha256y, prev_hash.data(), prev_hash.size());   // hash previous block pointer.
    SHA256_Final(const_cast<uint8_t*>(hash.data()), &sha256y);   //Finalizing and storing into hash
