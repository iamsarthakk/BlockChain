...
bool txn_t::validate()
{
    // TODO: This is  Homework Problem 1
    hash_result_t h1;
    SHA256_CTX sha256x;

    SHA256_Init(&sha256x);
    SHA256_Update(&sha256x, public_key.data(), public_key.size());
    SHA256_Final(const_cast<uint8_t*>(h1.data()), &sha256x);

    if (h1 != source_addr)
        return false;
    assert(h1 == source_addr);

    rsa_public_key_t pub_key(public_key.data(), public_key.size() );
    hash_result_t h2;
    SHA256_Init(&sha256x);
    SHA256_Update(&sha256x, public_key.data(), public_key.size());
    SHA256_Update(&sha256x, source_addr.data(), source_addr.size());
    SHA256_Update(&sha256x, dest_addr.data(), dest_addr.size());
    SHA256_Update(&sha256x, change_addr.data(), change_addr.size());
    SHA256_Update(&sha256x, &amount, sizeof(amount));
    SHA256_Final(const_cast<uint8_t*>(h2.data()), &sha256x);

    if(h2 == tx_hash && pub_key.verify(tx_hash.data(), tx_hash.size() ,tx_sign.data(),tx_sign.size()))
      return true;
    else
      return false;
